...
export const adminUpdateUser = async (req, res) => {
  ...
  if (email !== "") {
    emailMeetsConstraints = new RegExp(
  ...
  if (
    req.username === "admin" &&
    (isActive === false || !groups.includes("admin"))
  ) {
    return res
      .status(500)
      .json("Admin cannot be disabled or removed from the admin group")
  }
  ...
  groups =
    Array.isArray(groups) && groups.length !== 0 ? groups.join(",") : groups //if any change to groups, update groups
  ...
  if (!groups)
    updateUserQry = `UPDATE \`accounts\` SET \`password\`='${
      password ? password : foundUsers[0].password
    }', \`email\`='${email ? email : foundUsers[0].email}', \`isActive\`='${
      isActive ? 1 : 0
    }', \`groups\`=NULL WHERE \`username\`='${username}';`
  else
    updateUserQry = `UPDATE \`accounts\` SET \`password\`='${
      password ? password : foundUsers[0].password
    }', \`email\`='${email ? email : foundUsers[0].email}', \`isActive\`='${
      isActive ? 1 : 0
    }', \`groups\`='${groups}' WHERE \`username\`='${username}';`
  try {
...
export const adminCreateUser = async (req, res) => {
  ...
  try {
    const [foundUsers] = await sql.query(
    ...
    if (foundUsers.length >= 1) {
      return res
        .status(500)
        .json("Server error: Multiple entries with this username found.")
    }
    if (foundUsers.length === 1) {
...
export const updateUser = async (req, res) => {
  try {
    ...
    const emailMeetsConstraints =
      email &&
      email != "" &&
      new RegExp("^[a-zA-Z0-9]+@[a-zA-Z]+.[a-zA-Z]+$").test(email)
    ...
    password = password
      ? bcrypt.hashSync(password, bcrypt.genSaltSync(10))
      : foundUser.password //check for blank field, hash pw
...
export const getAllPermitDoneEmails = async (req, res) => {
  try {
    const results = await sql.query(
      `SELECT * FROM accounts WHERE isActive = 1 AND groupNames LIKE ?`,
      ["%" + group + "%"]
    )
    console.log("getAllPermitDoneEmails results[0]:" + results[0])
