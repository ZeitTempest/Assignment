...
function AddTaskDialog(props) {
  ...
  return (
    <div className="flex space-x-4 px-8 pb-8 justify-center w-auto">
      <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
        <h1 className="text-xl font-bold leading-tight tracking-tight md:text-2xl text-blue-900">Create Task for {appName}</h1>
        <div className="flex-col space-y">
          <div>Task Name</div>
          <input type="name" name="taskName" id="taskName" className="border text-white sm:text-sm rounded-lg block w-full p-2.5 bg-blue-gray-500 border-blue-gray-300 placeholder-gray-100 focus:bg-blue-gray-400 focus:ring-blue-500 focus:border-blue-500" placeholder="Task Name" required="" onChange={e => setTaskName(e.target.value)} />
          <div className="pt-4">Task Description</div>
          <TextField style={{ width: 400 }} readOnly multiline rows={8} value={props.description} onChange={e => setDescription(e.target.value)} />
        </div>
      </div>
      <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
        <div className="flex-col space-y">
        ...
        <button type="submit" onClick={handleSaveTask} className="w-2/5 self-auto text-white bg-teal-500 hover:bg-teal-700 focus:ring-blue-gray-200 my-2 focus:ring-4 focus:outline-none font-medium rounded-lg text-md px-5 py-2.5 text-center ">
          Save
        </button>
      </div>
    </div>
